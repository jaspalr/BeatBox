...
static void fillPlaybackBuffer(short *buff, int size)
{
	...
	pthread_mutex_lock(&audioMutex);
	 for(int i = 0; i < MAX_SOUND_BITES; i++){
		if(soundBites[i].pSound != NULL){
			...
			numSamples = soundBites[i].pSound->numSamples;
			data = soundBites[i].pSound->pData;
			for(int u =loc; u < numSamples; u++ ){
				value = data[u];
				if(value >  32767){
					value = 32767;
				}
	...
	Period_markEvent(PERIOD_EVENT_BEATS);
